...
float4 BarrierPass(float2 texCoord : TEXCOORD0) : COLOR
{
    ...
    float glow = exp(-pow((dist - uRadius) / 0.05, 2));

    float alpha = ring + glow * uGlowStrength;

    return float4(uColor, alpha * uOpacity);
}

technique DrawBarrier
{
    pass BarrierPass
    {
        PixelShader = compile ps_2_0 BarrierPass();
    }
}