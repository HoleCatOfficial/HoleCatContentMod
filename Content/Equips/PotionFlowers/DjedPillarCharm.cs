              ...
              // Here we take advantage of "player.eocDash" and "player.armorEffectDrawShadowEOCShield" to get the Shield of Cthulhu's afterimage effect
                ...
                if (Main.GameUpdateCount % 5 == 0)
                {
                    ...
                    if (skull != null)
                    {
                        for (int i = 0; i < 6; i++)
                        {
                            Projectile.NewProjectile(Player.GetSource_Accessory(skull), Main.rand.NextVector2FromRectangle(Player.Hitbox), Vector2.Zero, ProjectileID.SpiritFlame, 30, 8, Main.LocalPlayer.whoAmI);
                        }
                    }
