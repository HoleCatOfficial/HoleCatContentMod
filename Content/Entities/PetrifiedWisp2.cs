...
{
    public class PetrifiedWisp2 : ModNPC
    {
        ...
        public void DrawCrystalCore(SpriteBatch spriteBatch, Vector2 Center)
        {
            ...
            for (int i = 0; i < TrailPositions.Count; i++)
            {
                ...
                float scale = MathHelper.Lerp(2.6f, 0.0005f, progress);
			...
			{
				float progress = i / (float)TrailLength;
				float scale = MathHelper.Lerp(2f, 0.001f, progress);
        ...
        public override void HitEffect(NPC.HitInfo hit)
        {
            if (NPC.life <= 0)
            {
                for (int i = 0; i < 3; i++)
                {
                    NPC.NewNPC(NPC.GetSource_Death(), (int)NPC.Center.X, (int)NPC.Center.Y, ModContent.NPCType<PetrifiedWisp3>());
                }
            }
        }
    }
