...
namespace DestroyerTest.Content.Projectiles.Weapon.Melee
{
	...
	public class JnanaSwing : ModProjectile
	{
                ...
                for (int i = 0; i < ProjectileAmount; i++)
					{
						...
						Projectile.NewProjectile(Projectile.GetSource_FromThis(), swordTip, velocity * Main.rand.NextFloat(0.1f, 1.0f), ModContent.ProjectileType<KnowledgeButterfly>(), Projectile.damage / 2, 2, Main.player[Projectile.owner].whoAmI);
        ...
        {
            swordTip = Projectile.Center + Projectile.rotation.ToRotationVector2() * (Projectile.Size.Length() * Projectile.scale);
            PRTLoader.NewParticle(DTUtils.Fire[Main.rand.Next(DTUtils.Fire.Length)], swordTip, Vector2.Zero, new Color(182, 41, 172), 1, 60, 2);
            ...
            else if (CurrentAttack == AttackType.Spin)
            {
                ...
                if (Timer == (int)(execTime * SPINTIME * 3 / 4))
                {
                    ...
                    float angleIncrement = MathHelper.TwoPi / ProjectileAmount;
					PRTLoader.NewParticle(PRTLoader.GetParticleID<Boom2>(), swordTip, Vector2.Zero,  ColorLib.StellarFireGradientLooping(), 1);
					for (int i = 0; i < ProjectileAmount; i++)
					{
						Vector2 velocity = new Vector2(8, 0).RotatedBy(angleIncrement * i);
						swordTip = Projectile.Center + Projectile.rotation.ToRotationVector2() * (Projectile.Size.Length() * Projectile.scale);
