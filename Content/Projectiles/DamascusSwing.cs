...
namespace DestroyerTest.Content.Projectiles
{
	...
	public class DamascusSwing : ModProjectile
	{
		...
		private const float SPINTIME = 1.5f; // How much longer a spin is than a swing
		...
		public override string Texture => "DestroyerTest/Content/MeleeWeapons/DamascusRipper"; // Use texture of item as projectile texture
		...
		private void ExecuteStrike() {
			...
			if (CurrentAttack == AttackType.Swing) {
				...
				if (Timer < execTime / 2) {
					Vector2 forwardVelocity = new Vector2(player.direction, 0) * 1.5f; // Adjust the multiplier to control the launch speed
				...
				Lighting.AddLight(player.Center, 1.2f, 1.2f, 1.2f); // Adjust the color and intensity as needed
				...
				if (Timer == (int)(execTime * SPINTIME * 3 / 4)) {
					SoundEngine.PlaySound(new SoundStyle("DestroyerTest/Assets/Audio/DamascusSlash")); // Play sword sound again
					Projectile.ResetLocalNPCHitImmunity(); // Reset the local npc hit immunity for second half of spin
				}

				// Emit light during the spin
				Lighting.AddLight(player.Center, 1.0f, 0.5f, 0.0f); // Adjust the color and intensity as needed
