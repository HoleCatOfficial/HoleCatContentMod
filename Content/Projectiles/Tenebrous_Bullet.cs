...
namespace DestroyerTest.Content.Projectiles
{
	...
	public class Tenebrous_Bullet : ModProjectile
	{
        ...
        {
            SoundStyle Impact = new SoundStyle("DestroyerTest/Assets/Audio/TB_Impact");
            SoundEngine.PlaySound (Impact);

            // Spawn a bunch of smoke dusts.
			for (int i = 0; i < 30; i++) {
				Dust DeathDust = Dust.NewDustDirect(Projectile.position, 50, 50, ModContent.DustType<ShimmeringDust>(), 0f, 0f, 100, default, 1.5f);
				DeathDust.velocity *= 1.4f;
			}
        }
	}
}