...
{
public class ContenderSwordShot : ModProjectile
		{
        ...
        {
			target.AddBuff(ModContent.BuffType<HateShock>(), 60);
            // Correct entity source
            IEntitySource spawnSource = Projectile.GetSource_OnHit(target);
            
			if (damageDone > 1)
			{
            // Spawn the projectile
            Projectile.NewProjectile(spawnSource, target.Center, new Vector2(0, -25), ModContent.ProjectileType<ContenderSwordShot>(), damageDone - 25, 0f, Main.myPlayer);
			}
			if (damageDone < 1 && Projectile.type == ModContent.ProjectileType<ContenderSwordShot>())
			{
				Projectile.Kill();
