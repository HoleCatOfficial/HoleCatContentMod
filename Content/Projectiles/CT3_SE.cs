...
{
    public class CT3_SE : ModProjectile
    {
        public override string Texture => "DestroyerTest/Content/Projectiles/CT3SEProj1";
        private static Asset<Texture2D> chainTexture;
        public override void Load()
        {
            if (Main.dedServ)
                return;
            chainTexture = ModContent.Request<Texture2D>("DestroyerTest/Content/Projectiles/CT3SEChain");
        }
            ...
            if (Main.myPlayer == Projectile.owner && --pauseTimer <= 0)
            {
                switch (Projectile.ai[0])
                {
                    case 0:
                        ...
                        if (!Main.dedServ)
                            SoundEngine.PlaySound(new SoundStyle($"DestroyerTest/Assets/Audio/ConstitutionT3Slash") with { Volume = 0.3f, Pitch = 0.4f }, Projectile.position);
                    ...
                    case 4:
                        ...
                        else
                        {
                            ...
                            if (!Main.dedServ)
                                SoundEngine.PlaySound(new SoundStyle($"DestroyerTest/Assets/Audio/ConstitutionT3Slash") with { Volume = 0.3f, Pitch = 0.4f }, Projectile.position);
                            Timer = 0;
                            startVector = PolarVector(1, (Main.MouseWorld - player.Center).ToRotation());
                            Projectile.netUpdate = true;
                        }
                        break;
                    case 5:
                        ...
                        if (Timer >= 24 * SwingSpeed)
                        {
                            ...
                            if (Main.MouseWorld.X < player.Center.X)
                                ...
                                player.direction = 1;
                            extendoLength = 0;
                            Projectile.velocity = PolarVector(5, (Main.MouseWorld - player.Center).ToRotation());
                            Projectile.alpha = 255;
                            startVector = PolarVector(1, (Main.MouseWorld - player.Center).ToRotation() - ((MathHelper.PiOver2 + 0.6f) * player.direction));
                            vector = startVector * Length;
                            Projectile.ai[0] = 1;
                            SoundEngine.PlaySound(SoundID.Item1, Projectile.position);
                            if (!Main.dedServ)
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            for (int i = 0; i < 16; i++)
                Dust.NewDustPerfect(target.Center + directionTo * 5 + new Vector2(0, 35) + player.velocity, DustID.Wraith, directionTo.RotatedBy(Main.rand.NextFloat(-0.5f, 0.5f) + 3.14f) * Main.rand.NextFloat(3f, 5f) + (player.velocity / 2), 0, Color.Red * .8f, 1.6f);
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Texture2D segMid = ModContent.Request<Texture2D>("DestroyerTest/Content/Projectiles/CT3SEProj2").Value;
            Texture2D segEnd = ModContent.Request<Texture2D>("DestroyerTest/Content/Projectiles/CT3SEProj3").Value;
            Vector2 origin = new(texture.Width / 2f, texture.Height / 2f);
            Vector2 armCenter = player.RotatedRelativePoint(player.MountedCenter, true) + new Vector2(-player.direction * 4, -4);
            Vector2 v = PolarVector(40, (Projectile.Center - armCenter).ToRotation());
            ...
            for (int i = 0; i < 8; i++)
            {
                Main.EntitySpriteDraw(i < 7 ? segMid : segEnd, Projectile.Center - v + PolarVector(extendoLength * extendo[i], (Projectile.Center - armCenter).ToRotation()) - Main.screenPosition, null, Projectile.GetAlpha(lightColor), Projectile.rotation, origin, Projectile.scale, spriteEffects, 0);
            }
            return false;
        }
        public override bool PreDrawExtras()
        {
            ...
            Vector2 center = Projectile.Center - PolarVector(30, (Projectile.Center - armCenter).ToRotation()) + PolarVector(extendoLength * extendo[7], (Projectile.Center - armCenter).ToRotation());
