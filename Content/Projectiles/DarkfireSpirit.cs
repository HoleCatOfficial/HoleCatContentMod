...
namespace DestroyerTest.Content.Projectiles
{
    public class DarkFireSpirit : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            Fire fire = new Fire();
            fire.PrepareFire(Projectile.Center, Vector2.Zero, DTUtils.RandomDirection(2), Main.rand.NextFloat(-0.3f, 0.3f), Color.Indigo, 0.5f, 100, FireDrawMode.Additive);
            ParticleEngine.BehindProjectiles.Add(fire);

            Fire fireX = new Fire();
            fireX.PrepareFire(Projectile.Center, Vector2.Zero, DTUtils.RandomDirection(2), Main.rand.NextFloat(-0.3f, 0.3f), DTColorUtils.Pastel(Color.Purple, 0.4f), 0.3f, 100, FireDrawMode.Additive);
            ParticleEngine.BehindProjectiles.Add(fireX);

            if (Main.rand.NextBool(4))
            {
                Fire fire2 = new Fire();
                fire2.PrepareFire(Projectile.Center, Vector2.Zero, DTUtils.RandomDirection(2), Main.rand.NextFloat(-0.3f, 0.3f), Color.DarkMagenta, 0.5f, 100, FireDrawMode.Additive);
                ParticleEngine.BehindProjectiles.Add(fire2);

                Fire fire3 = new Fire();
                fire3.PrepareFire(Projectile.Center, Vector2.Zero, DTUtils.RandomDirection(2), Main.rand.NextFloat(-0.3f, 0.3f), DTColorUtils.Pastel(Color.HotPink, 0.5f), 0.35f, 100, FireDrawMode.Additive);
