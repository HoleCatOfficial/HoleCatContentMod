...
{
    internal class RiftGreatswordProjectile : ModProjectile
    {
	...
	SoundStyle Swing = new SoundStyle("DestroyerTest/Assets/Audio/RiftGreatSword");
    private enum AttackType // Which attack is being performed
        ...
        SpinSlash // New spin slash attack
        }
    ...
    public override void SetStaticDefaults()
        ...
        {
            Projectile.width = 92; // Hitbox width of projectile
            Projectile.height = 92; // Hitbox height of projectile
    ...
    public override string Texture => "DestroyerTest/Content/Projectiles/RiftGreatswordProjectile";
    ...
    public override void AI()
        ...
        switch (CurrentStage)
        {
            case AttackStage.Prepare:
                if (CurrentAttack == AttackType.SpinSlash)
                    PrepareSpinSlash();
                else
                    PrepareStrike();
                break;
    ...
    private void PrepareStrike()
        ...
        if (Timer >= prepTime)
            {
            SoundEngine.PlaySound(Swing); // Play sword sound here since playing it on spawn is too early
    ...
    private void PrepareSpinSlash()
        {
            Progress = MathHelper.SmoothStep(0, MathHelper.PiOver2, Timer / prepTime); // Smoothly rotate to a starting position
            Size = MathHelper.SmoothStep(0, 1.2f, Timer / prepTime); // Slightly overgrow the sword for a charging effect

            if (Timer >= prepTime)
            {
                SoundEngine.PlaySound(Swing); // Play sword sound
                CurrentStage = AttackStage.Execute; // Move to execution stage
            }
        }

        // Updated method for executing the spin slash
        private void ExecuteSpinSlash()
        {
            SoundStyle Spin = new SoundStyle("DestroyerTest/Assets/Audio/Destitute");
            SoundEngine.PlaySound(Spin, Owner.position);

            // Add a slight overstep effect to the rotation
            float overstepFactor = 1.1f; // Slightly exceed a full rotation
            Progress = MathHelper.TwoPi * overstepFactor * (Timer / execTime); // Smooth rotation with overstep

            Owner.velocity = Vector2.Lerp(Owner.velocity, (Main.MouseWorld - Owner.Center).SafeNormalize(Vector2.Zero) * 65f, 0.1f); // Propel player toward cursor
            Owner.fullRotation = Progress; // Rotate the player sprite
            Owner.fullRotationOrigin = new Vector2(Owner.width / 2, Owner.height / 2); // Set rotation origin to the center of the player
        ...
        // Updated method for unwinding the spin slash
        private void UnwindSpinSlash()
        {
            // Smoothly decelerate the rotation and shrink the sword
            Progress = MathHelper.SmoothStep(MathHelper.TwoPi, 0, Timer / hideTime); // Decelerate rotation
            Size = MathHelper.SmoothStep(1.2f, 0, Timer / hideTime); // Shrink sword size smoothly

            Owner.fullRotation = Progress; // Gradually reset player rotation
            if (Timer >= hideTime)
            {
                Owner.fullRotation = 0f; // Ensure player rotation is fully reset
                Projectile.Kill();
            }
        }
