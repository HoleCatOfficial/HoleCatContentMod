...
namespace DestroyerTest.Content.Projectiles
{
	...
	public class RiftTeardrop_Thrown : ModProjectile
	{
		...
		public override void SetDefaults()
		{
			...
			Projectile.penetrate = 8; // How many monsters the projectile can penetrate.
		...
		private const int GravityDelay = 125;
		...
		private void NormalAI() {
			...
			if (Main.rand.NextBool(3)) {
				Dust dust = Dust.NewDustDirect(Projectile.position, Projectile.height, Projectile.width, ModContent.DustType<RiftDust>(), Projectile.velocity.X * .2f, Projectile.velocity.Y * .2f, 200, Scale: 1.2f);
				dust.velocity += Projectile.velocity * 0.3f;
				dust.velocity *= 0.2f;
			}
			if (Main.rand.NextBool(4)) {
				Dust dust = Dust.NewDustDirect(Projectile.position, Projectile.height, Projectile.width, ModContent.DustType<RiftDust>(),
					0, 0, 254, Scale: 0.3f);
		...
		public override void OnKill(int timeLeft) {
			...
			for (int i = 0; i < 20; i++) {
                ...
                if (i == 0) {
                    ...
                    flash.rotation = Main.rand.NextFloat(MathHelper.TwoPi); // Random initial rotation
                    flash.scale = 1.5f; // Adjust the scale if needed
                }
			}
			...
			if (Projectile.owner == Main.myPlayer) {
				...
				if (Main.rand.NextBool(18)) {
					item = Item.NewItem(Projectile.GetSource_DropAsItem(), Projectile.getRect(), ModContent.ItemType<RiftMaker>());
				}
