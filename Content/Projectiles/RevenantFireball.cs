...
namespace DestroyerTest.Content.Projectiles
{
    ...
    public class RevenantFireball : ModProjectile
    {
        ...
        {
            int Rad = 24;
            Dust.NewDust(Projectile.position, (int)Projectile.Center.X + Rad, (int)Projectile.Center.Y + Rad, DustID.WaterCandle, Scale: 2f);
            SoundEngine.PlaySound(SoundID.Item14, Projectile.position);
            foreach (NPC target in Main.npc)
            {
                if (target.active && !target.friendly && target.life > 0 && Projectile.Distance(target.Center) < Rad)
                {
                    NPC.HitInfo hit = new NPC.HitInfo()
                    {
                        DamageType = ModContent.GetInstance<ScepterClass>(),
                        Damage = 4,
                        Knockback = 6f,
                        HitDirection = 0
                    };
                    target.StrikeNPC(hit, false, true);
                }
            }
        }
    }
