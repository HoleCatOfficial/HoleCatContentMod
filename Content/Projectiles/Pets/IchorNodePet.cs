...
{
    [AutoloadGlowmask]
    public class IchorNodePet : ModProjectile
    {
        ...
        public override void OnSpawn(IEntitySource source)
        {
            ...
            for (int f = 0; f < 8; f++)
            {
                ...
                PRTLoader.NewParticle(PRTLoader.GetParticleID<SimpleParticle>(), Projectile.Center, Dir, ColorLib.Ichor, Main.rand.NextFloat(0.15f, 1f));
            }
        }
        ...
        private void DoIdleMovement(Player player)
        {
            ...
            int cursedType = ModContent.ProjectileType<CursedNodePet>();
            Projectile other = null;
            for (int i = 0; i < Main.maxProjectiles; i++)
            {
                Projectile p = Main.projectile[i];
                if (p.active && p.owner == Projectile.owner && p.type == cursedType && p.whoAmI != Projectile.whoAmI)
                {
                    other = p;
                    break;
                }
