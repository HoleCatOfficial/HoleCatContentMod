...
{
	public class ScepterOfVespae : ModItem
	{
        ...
        public override void SetDefaults() {
			...
			Item.damage = 18 + (int)Math.Round(ScepterClassStats.DamageModifier); // The damage of your sword, this is dynamically adjusted in the projectile code.
			...
			Item.shoot = ProjectileID.Bee; // The sword as a projectile
            Item.shootSpeed = 10f;
		...
		public override bool CanUseItem(Player player) {
			...
			if (player.altFunctionUse == 2) {
				...
				Item.damage = 18 + 10 + (int)Math.Round(ScepterClassStats.DamageModifier);
				Item.shoot = ModContent.ProjectileType<ScepterOfVespaeThrown>(); // The projectile is what makes a shortsword work
			...
			Item.useStyle = ItemUseStyleID.Shoot;
            Item.UseSound = SoundID.Item17;
			Item.damage = 10 + (int)Math.Round(ScepterClassStats.DamageModifier);
			Item.knockBack = 0;  // The knockback of your sword, this is dynamically adjusted in the projectile code.
			Item.autoReuse = true; // This determines whether the weapon has autoswing
			Item.DamageType = ModContent.GetInstance<ScepterClass>();
            Item.crit = 10; // The critical strike chance the weapon has. The player, by default, has a 4% critical strike chance.ion
		...
		public override bool? UseItem(Player player)
		{
			if (player.altFunctionUse == 2) // Throwing mode
			{
				...
				foreach (Projectile proj in Main.projectile)
				{
					...
					if (proj.active && proj.owner == player.whoAmI && proj.type == ModContent.ProjectileType<ScepterOfVespaeThrown>())
					{
						// Safely get the existenceTimer value
						if (proj.ModProjectile is ScepterOfVespaeThrown thrownProj)
						{
	...
	public class SoV_DROP_NPC : GlobalNPC
	{
		public override void ModifyNPCLoot(NPC npc, NPCLoot npcLoot) {
			...
			if (npc.type == NPCID.Hornet) {
				npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<ScepterOfVespae>(), 50, 1, 1));
			}
		}
	}
} 