...
{
	public class CelestialDiscord : ModItem
	{
        ...
        public override void SetDefaults() {
			...
			Item.shoot = ModContent.ProjectileType<SolarDart>(); // The sword as a projectile
            Item.shootSpeed = 20f;
		...
		public override bool CanUseItem(Player player) {
			...
			if (player.altFunctionUse == 2) {
				...
				Item.damage = 65 + 10 + (int)Math.Round(ScepterClassStats.DamageModifier); // The damage of your sword, this is dynamically adjusted in the projectile code.
				Item.shoot = ModContent.ProjectileType<CelestialDiscordThrown>(); // The projectile is what makes a shortsword work
			...
			Item.useStyle = ItemUseStyleID.Shoot;
            Item.UseSound = SoundID.NPCDeath55;
			Item.damage = 65 + (int)Math.Round(ScepterClassStats.DamageModifier); // The damage of your sword, this is dynamically adjusted in the projectile code.
			Item.knockBack = 0;  // The knockback of your sword, this is dynamically adjusted in the projectile code.
			Item.autoReuse = true; // This determines whether the weapon has autoswing
			Item.DamageType = ModContent.GetInstance<ScepterClass>();
            Item.crit = 10; // The critical strike chance the weapon has. The player, by default, has a 4% critical strike chance.ion
		...
		public override bool? UseItem(Player player)
		{
			if (player.altFunctionUse == 2) // Throwing mode
			{
				...
				foreach (Projectile proj in Main.projectile)
				{
					...
					if (proj.active && proj.owner == player.whoAmI && proj.type == ModContent.ProjectileType<CelestialDiscordThrown>())
					{
						// Safely get the existenceTimer value
						if (proj.ModProjectile is CelestialDiscordThrown thrownProj)
						{
		...
		public override void AddRecipes() {
			CreateRecipe()
				...
				.AddIngredient<EnchantedScepter>(4)
