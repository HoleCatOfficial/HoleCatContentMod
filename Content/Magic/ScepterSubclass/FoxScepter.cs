...
{
	public class FoxScepter : ModItem
	{
        ...
        public override void SetDefaults() {
			...
			Item.damage = 10 + (int)Math.Round(ScepterClassStats.DamageModifier); // The damage of your sword, this is dynamically adjusted in the projectile code.
			...
			Item.shoot = ModContent.ProjectileType<PipeBomb>(); // The sword as a projectile
            Item.shootSpeed = 10f;
		...
		public override bool CanUseItem(Player player) {
			...
			if (player.altFunctionUse == 2) {
				...
				Item.damage = 10 + 10 + (int)Math.Round(ScepterClassStats.DamageModifier);
				Item.shoot = ModContent.ProjectileType<FoxScepterThrown>(); // The projectile is what makes a shortsword work
		...
		public override bool? UseItem(Player player)
		{
			if (player.altFunctionUse == 2) // Throwing mode
			{
				...
				foreach (Projectile proj in Main.projectile)
				{
					...
					if (proj.active && proj.owner == player.whoAmI && proj.type == ModContent.ProjectileType<FoxScepterThrown>())
					{
						// Safely get the existenceTimer value
						if (proj.ModProjectile is FoxScepterThrown thrownProj)
						{
