...
namespace DestroyerTest.Content.Magic.ScepterSubclass
{
	public class NatureScepter : ModItem
	{
        ...
        public override void SetDefaults() {
			...
			Item.shoot = ModContent.ProjectileType<NatureShot>(); // The sword as a projectile
            Item.shootSpeed = 10f;
		...
		public override bool CanUseItem(Player player) {
			...
			if (player.altFunctionUse == 2) {
				...
				Item.shoot = ModContent.ProjectileType<NatureScepterThrown>(); // The projectile is what makes a shortsword work
		...
		public override bool? UseItem(Player player)
		{
			if (player.altFunctionUse == 2) // Throwing mode
			{
				...
				foreach (Projectile proj in Main.projectile)
				{
					...
					if (proj.active && proj.owner == player.whoAmI && proj.type == ModContent.ProjectileType<NatureScepterThrown>())
					{
						// Safely get the existenceTimer value
						if (proj.ModProjectile is NatureScepterThrown thrownProj)
						{
