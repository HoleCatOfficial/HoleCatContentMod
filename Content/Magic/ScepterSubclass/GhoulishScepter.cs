...
{
	public class GhoulishScepter : ModItem
	{
        ...
        public override void SetDefaults() {
            ...
            Item.UseSound = new SoundStyle("DestroyerTest/Assets/Audio/GhoulishScepter/Ghost", 6) with
            {
                PitchVariance = 1.0f
            };
			...
			Item.shoot = ModContent.ProjectileType<GhoulProjectile>(); // The sword as a projectile
            Item.shootSpeed = 20f;
		...
		public override bool CanUseItem(Player player) {
			...
			if (player.altFunctionUse == 2) {
				...
				Item.damage = 45 + 10 + (int)Math.Round(ScepterClassStats.DamageModifier); // The damage of your sword, this is dynamically adjusted in the projectile code.
				Item.shoot = ModContent.ProjectileType<GhoulishScepterThrown>(); // The projectile is what makes a shortsword work
			...
			Item.damage = 45 + (int)Math.Round(ScepterClassStats.DamageModifier); // The damage of your sword, this is dynamically adjusted in the projectile code.
			Item.knockBack = 0;  // The knockback of your sword, this is dynamically adjusted in the projectile code.
			Item.autoReuse = true; // This determines whether the weapon has autoswing
			Item.DamageType = ModContent.GetInstance<ScepterClass>();
            Item.crit = 10; // The critical strike chance the weapon has. The player, by default, has a 4% critical strike chance.ion
		...
		public override bool? UseItem(Player player)
		{
			if (player.altFunctionUse == 2) // Throwing mode
			{
				...
				foreach (Projectile proj in Main.projectile)
				{
					...
					if (proj.active && proj.owner == player.whoAmI && proj.type == ModContent.ProjectileType<GhoulishScepterThrown>())
					{
						// Safely get the existenceTimer value
						if (proj.ModProjectile is GhoulishScepterThrown thrownProj)
						{
    ...
    public class GhoulScep_DROP_NPC : GlobalNPC
	{
		public override void ModifyNPCLoot(NPC npc, NPCLoot npcLoot) {
			...
			if (npc.type == ModContent.NPCType<PossessedScepter>()) {
