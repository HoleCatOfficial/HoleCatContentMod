...
{
	public class ObsidianOculus : ModItem
	{
        ...
        public override void SetDefaults() {
			...
			Item.shoot = ModContent.ProjectileType<ObsidianShard>(); // The sword as a projectile
            Item.shootSpeed = 20f;
		...
		public override bool CanUseItem(Player player) {
			...
			if (player.altFunctionUse == 2) {
				...
				Item.damage = 15 + 10 + (int)Math.Round(ScepterClassStats.DamageModifier);
				Item.shoot = ModContent.ProjectileType<ObsidianOculusThrown>(); // The projectile is what makes a shortsword work
		...
		public override bool? UseItem(Player player)
		{
			if (player.altFunctionUse == 2) // Throwing mode
			{
				...
				foreach (Projectile proj in Main.projectile)
				{
					...
					if (proj.active && proj.owner == player.whoAmI && proj.type == ModContent.ProjectileType<ObsidianOculusThrown>())
					{
						// Safely get the existenceTimer value
						if (proj.ModProjectile is ObsidianOculusThrown thrownProj)
						{
