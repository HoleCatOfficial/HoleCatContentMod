...
{
	public class LivingWoodCane : ModItem
	{
        ...
        public override void SetDefaults() {
			...
			Item.damage = 3 + (int)Math.Round(ScepterClassStats.DamageModifier); // The damage of your sword, this is dynamically adjusted in the projectile code.
		...
		public override bool CanUseItem(Player player) {
			...
			if (player.altFunctionUse == 2) {
				...
				Item.damage = 3 + 10 + (int)Math.Round(ScepterClassStats.DamageModifier);
				Item.shoot = ModContent.ProjectileType<LivingWoodCaneThrown>(); // The projectile is what makes a shortsword work
			...
			Item.useStyle = ItemUseStyleID.Shoot;
            Item.UseSound = SoundID.Item25;
			Item.damage = 3 + (int)Math.Round(ScepterClassStats.DamageModifier);
			Item.knockBack = 0;  // The knockback of your sword, this is dynamically adjusted in the projectile code.
			Item.autoReuse = true; // This determines whether the weapon has autoswing
			Item.DamageType = ModContent.GetInstance<ScepterClass>();
            Item.crit = 10; // The critical strike chance the weapon has. The player, by default, has a 4% critical strike chance.ion
		...
		public override bool? UseItem(Player player)
		{
			if (player.altFunctionUse == 2) // Throwing mode
			{
				...
				foreach (Projectile proj in Main.projectile)
				{
					...
					if (proj.active && proj.owner == player.whoAmI && proj.type == ModContent.ProjectileType<LivingWoodCaneThrown>())
					{
						// Safely get the existenceTimer value
						if (proj.ModProjectile is LivingWoodCaneThrown thrownProj)
						{
