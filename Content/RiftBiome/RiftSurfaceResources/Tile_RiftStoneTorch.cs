...
namespace DestroyerTest.Content.RiftBiome.RiftSurfaceResources
{
	...
	public class Tile_RiftStoneTorch : ModTile
	{
		...
		public override void SetStaticDefaults() {
			...
			AddMapEntry(ColorLib.Rift, Language.GetText("ItemName.Torch"));
        ...
        private bool Rift(Player player)
        {
            if(player.InModBiome<RiftSurface>() || 
            player.InModBiome<RiftDesert>() ||
            player.InModBiome<RiftTundra>() ||
            player.InModBiome<RiftUnderground>() ||
            player.InModBiome<RiftDesertUnderground>())
            {
                return true;
            }
		...
		public override float GetTorchLuck(Player player) {
			...
			return Rift(player) ? 1f : -0.1f; // ExampleTorch gives maximum positive luck when in example biome, otherwise a small negative luck
		}
		...
		public override void EmitParticles(int i, int j, Tile tileCache, short tileFrameX, short tileFrameY, Color tileLight, bool visible) {
			...
			if (Main.rand.NextBool(40) && tileFrameX < 66) {
				int dustChoice = ModContent.DustType<ColorableNeonDust>();
				Dust dust;
				...
				dust = Dust.NewDustDirect(new Vector2(i * 16 + 4, j * 16), 4, 4, dustChoice, 0f, 0f, 100, ColorLib.Rift);
