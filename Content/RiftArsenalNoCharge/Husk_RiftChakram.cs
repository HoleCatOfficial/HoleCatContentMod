...
{
	public class Husk_RiftChakram : ModItem
	{
        ...
        public override void SetDefaults() {
			...
			Item.damage = 380; // The damage of your sword, this is dynamically adjusted in the projectile code.
			Item.DamageType = DamageClass.Ranged;
            Item.crit = 30; // The critical strike chance the weapon has. The player, by default, has a 4% critical strike chance.ion
			Item.shoot = ModContent.ProjectileType<Husk_RiftChakramThrown>(); // The sword as a projectile
            Item.shootSpeed = 20f;
		...
		public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
		{
			...
			{
				if (proj.active && proj.owner == player.whoAmI && proj.type == ModContent.ProjectileType<Husk_RiftChakramThrown>())
				{
        ...
        {
			Item.NewItem(player.GetSource_OpenItem(Type), player.position, ModContent.ItemType<RiftChakram>());
            Item.TurnToAir();
