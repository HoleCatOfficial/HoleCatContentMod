...
{
	public class Tile_RiftCache : ModTile
	{
		public override void SetStaticDefaults() {
			...
			RegisterItemDrop(ModContent.ItemType<Item_RiftCache>(), 1);
		...
		*/
		...
		public override bool UnlockChest(int i, int j, ref short frameXAdjustment, ref int dustType, ref bool manual) {
			if (Main.eclipse) {
				Main.NewText("The Eclipse's energy disturbances impede the decoder from working properly...", Color.Orange);
				return false;
			}
		...
		public override bool RightClick(int i, int j) {
            ...
            SoundStyle DecodeSound = new SoundStyle($"DestroyerTest/Assets/Audio/PasswordDecoding") with {
				Volume = 1.0f, 
    			Pitch = 0.0f, 
    			PitchVariance = 0.5f, 
			};
			...
			if (Main.netMode == NetmodeID.MultiplayerClient && !isLocked) {
				if (left == player.chestX && top == player.chestY && player.chest != -1) {
					player.chest = -1;
					Recipe.FindRecipes();
					SoundEngine.PlaySound(DecodeSound);
				}
				else {
				...
				if (isLocked) {
					...
					int key = ModContent.ItemType<PasswordDecoder>();
					...
					if (chest != -1) {
						...
						if (chest == player.chest) {
							...
							SoundEngine.PlaySound(player.chest < 0 ? SoundID.Item15 : SoundID.MenuTick);
		...
		public override void MouseOver(int i, int j) {
			...
			if (chest < 0) {
				...
				if (player.cursorItemIconText == defaultName) {
					player.cursorItemIconID = ModContent.ItemType<Item_RiftCache>();
					if (Main.tile[left, top].TileFrameX / 36 == 1) {
						player.cursorItemIconID = ModContent.ItemType<PasswordDecoder>();
					}
